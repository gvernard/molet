...
c      double precision defl(2),magmx2(2,2),magmx(2,2),
c     *     defl2(2),defl3(2)
c
c      do i=1,100
c 10      write(*,*) 'Enter axis ratio (<=1; <0 means: end):'
c         read(*,*) arat
c         if (arat .lt. 0.d0) goto 30
c         if (arat .gt. 1.d0) then
c            write(*,*) 'Illegal ratio, try again'
c            goto 10
c         endif
c 12      write(*,*) 'Enter q,gam,s2'
c         read(*,*) q,gam,s2
c         if (s2 .lt. 0.d0) then
c            write(*,*) 'Illegal (core rad.)^2, try again'
c            goto 12
c         endif
c         write(*,*) 'Now, evaluation loop'
c         do j=1,100
c            write(*,*) 'Enter x1,x2 (both = -100 to exit loop):'
c            read(*,*) x1,x2               
c            if ((x1+100.d0)*(x1+100.d0)+(x2+100.d0)*(x2+100.d0) 
c     *           .lt. 1.d-10) goto 20
c            call fastelldefl(x1,x2,q,gam,arat,s2,defl3)
c            write(*,*) 'fastell Defl.= ',defl3
c            call fastellmag(x1,x2,q,gam,arat,s2,defl,magmx)
c            write(*,*) 'fastell mag routine: Defl.= ',defl
c            write(*,*) 'and Magmx(11,22,12)= ',magmx(1,1)
c            write(*,*) magmx(2,2),magmx(1,2)
c            call ellipdefl(x1,x2,q,gam,arat,s2,defl2)
c            write(*,*) 'slow Defl.= ',defl2
c            call ellipmag(x1,x2,q,gam,arat,s2,defl2,magmx2)
c            write(*,*) 'slow mag routine: Defl.= ',defl2
c            write(*,*) 'and Magmx(11,22,12)= ',magmx2(1,1)
c            write(*,*) magmx2(2,2),magmx2(1,2)
c            call ellipphi(x1,x2,q,gam,arat,s2,phi)
c            write(*,*) 'slow Phi= ',phi
c         enddo
c 20   enddo
c 30   end
